...
# Changed to run on cluster

fp <- '~/data/forestlight'
dat <- read.csv(file.path(fp, 'GrowthLightMerged.csv'), stringsAsFactors = FALSE)

# Load shade tolerance.
comita <- read.csv(file.path(fp, 'ComitaAppendix1.csv'), stringsAsFactors = FALSE)
laselva <- read.csv(file.path(fp, 'shade_tolerance_la_selva.csv'), stringsAsFactors = FALSE)
harv <- read.csv(file.path(fp, 'shade_tolerance_harvard.csv'), stringsAsFactors = FALSE)
