...
(p_rich <- ggplot(bin_x_fg %>% filter(!fg %in% 'unclassified' & richness > 0), aes(x = bin_midpoint, y = richness, color = fg)) + 
  geom_point(size = 3) + scale_x_log10(name = 'diameter') + theme_plant() +
  scale_color_manual(values = guild_colors) +
  theme(legend.position = 'right'))

# Log y scale
(p_rich_log <- p_rich + scale_y_log10())
...
(p_ratio <- ggplot(richness_wide, aes(x = bin_midpoint)) +
  geom_point(size = 3, color = 'gray', aes(y = richness_ratio_fastslow)) +
  geom_point(size = 3, color = 'black', aes(y = richness_ratio_pioneerbreeder)) +
  ...
  scale_x_log10(name = 'diameter') + scale_y_log10(name = 'richness ratio') + theme_plant())
