...
- **7 Feb 2017**: Changed to midpoints of bins instead of medians, changed to log-log plot.
...
ggplot(bci_bybin, aes(x = mid_biomass, 
                      y = CII_mean, 
...
ggplot(harv_bybin, aes(x = mid_biomass, 
                       y = CII_mean, 
                       ymin = CII_mean - CII_se, 
                       ymax = CII_mean + CII_se)) +
  geom_pointrange() +
...
```{r}
print(bci_bybin[,c(2,4,5,6,9)], n = nrow(bci_bybin))
```

\newpage
```{r}
print(harv_bybin[,c(2,4,5,6,9)], n = nrow(harv_bybin))
```
