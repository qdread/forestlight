...
# Edut 04 Apr: Correct the formula for power law.
...
dens_prod_ci <- function(fit, dbh_pred, dens_form, prod_form, total_prod, x_min = NULL, n_indiv = 1, ll = 1.1, ul = 316, delete_samples = NULL) {
  ...
  powerlaw_exp_log <- function(x, a, b, c, beta0, beta1) exp(-beta0 + beta1*log(x) + log(-a*x^(-b)+c))
  powerlaw_log <- function(x, beta0, beta1) exp(-beta0 + beta1*log(x))
