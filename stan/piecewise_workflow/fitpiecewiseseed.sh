...
# Include (random) seed with the -v flag
# chain is the PBS_ARRAYID

module load GNU/6.2

((seed=seed+PBS_ARRAYID))

~/forestlight/stancode/model_d${densitymodel}p${productionmodel} sample algorithm=hmc engine=nuts max_depth=20 num_samples=${NS} num_warmup=${NW} thin=1 adapt delta=0.9 random seed=${seed} data file=~/forestlight/stanrdump/${dumptype}_${guild}_${year}.r output file=~/forestlight/stanoutput/fit_d${densitymodel}p${productionmodel}_${guild}_${year}_${PBS_ARRAYID}.csv
